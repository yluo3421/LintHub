...
//  QUESTION: why does it not require async logic?
  let sumitSucceeded =  document.getElementsByClassName('text-green-s dark:text-dark-green-s flex items-center gap-2 text-[16px] font-medium leading-6');
  console.log(sumitSucceeded);
if (sumitSucceeded) {
    console.log("!!!!!!!!!!!!!!!!!!! targeted element found.")
    setTimeout(function () {
      /* Only post if post button was clicked and url changed */

      // curent print value: false. Expected: true
      console.log("oldPath !== window.location.pathname:")
      console.log(oldPath !== window.location.pathname);

      // curent print value: true. Expected: true
      console.log("oldPath === window.location.pathname.substring(0, oldPath.length):")
      console.log(oldPath === window.location.pathname.substring(0, oldPath.length));

      // curent print value: true. Expected: false
      console.log("!Number.isNaN(window.location.pathname.charAt(oldPath.length)):")
      console.log(!Number.isNaN(window.location.pathname.charAt(oldPath.length)));
